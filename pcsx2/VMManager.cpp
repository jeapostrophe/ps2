...
{
	// The state restores the game flags and ElfCRC, not the ELF info the boot
	// reads from the disc (ElfEntry, DiscSerial, LastELF). A core whose own boot
	// never reached the ELF still has cpuReset's -- ElfEntry -1, no serial --
	// which GAME_LOADING waits on and UpdateRunningGame keys the GameDB by, so
	// read it as the boot does. A disc that names a serial but yields no
	// ElfEntry (PS1, an unloadable ELF) is read once, not on every load.
	if ((g_GameLoading || g_GameStarted) && ElfEntry == 0xFFFFFFFF && !DiscSerial[0])
		Internal::ReloadBootElfInfo();
...
void VMManager::Internal::ReloadBootElfInfo()
{
	if (!s_elf_override.empty())
